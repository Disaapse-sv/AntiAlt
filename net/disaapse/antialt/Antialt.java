...
import net.disaapse.antialt.ignoreCommand;
import net.disaapse.antialt.util.Config;
...
public class Antialt extends JavaPlugin {
	...
	public void onEnable() {
		// リスナー登録
		getServer().getPluginManager().registerEvents(new AntialtListener(), this);
		// コマンドの登録
		...
		try {
			...
			urlCon.setRequestProperty("User-Agent", "Antialt");
...
//			String[] banlist = {"{\"uuid\":\"58baabf2-4a5f-4906-b6ec-0af2b832688e\", \"name\":\"CrossHearts\", \"reason\":\"Compromised Account\"}"};
			...
			Bukkit.getServer().getConsoleSender().sendMessage(ChatColor.GREEN + "チェックリストの読み込みに成功");
		} catch (Exception e) {
			Bukkit.getServer().getConsoleSender().sendMessage(ChatColor.RED + "チェックリストの読み込みに失敗");
			e.printStackTrace();
		}
	}
